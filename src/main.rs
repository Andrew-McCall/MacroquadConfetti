...
impl Ball {
    fn new(x: f32, y:f32, seed:usize) -> Ball{   
        ...
        return Ball{id, x, y, x_vel, y_vel:0.0, color:Color::from_rgba((random as usize*47%255) as u8, (random as usize*29%255) as u8, (random as usize*101%255) as u8, 255)}
    }
    ...
    fn tick(&mut self){
        ...
        self.y_vel = if self.y_vel.is_normal() {self.y_vel} else {0.0};
        self.x_vel = self.x_vel.clamp(-MAX_VEL, MAX_VEL);
        self.x_vel = if self.x_vel.is_normal() {self.x_vel} else {0.0};
